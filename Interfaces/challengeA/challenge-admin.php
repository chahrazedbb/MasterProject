<?php
    ...
    if (isset($_POST['delete'])) {
        $ideaid = $_POST['ideaid'];
        $sqll2 = ("INSERT INTO removed_idea (id, what_about, how_it_works, when_it_works, member_id, time, session, idea_date) 
                   select id, what_about, how_it_works, when_it_works, member_id, time, session, idea_date 
                    from idea 
                    WHERE '$ideaid'=id
                   ");
...
<html>
    ...
    <div>
        ...
        <div style="margin-left : 2% ;">
            ...
            <p><b>Total number of new ideas : </b><?php $row4 = $conn->query($sql4)->fetch(); echo $row4['ideas']-29; ?>
            </p>
             ...
             if(isset($_POST['ideas'])){
               $current_cat = null; 
                foreach($conn->query($sql) as $row) { 
                  if ($row["member_id"] != $current_cat) {
                    $current_cat = $row["member_id"];
         ...
         ?>
            ...
            <?php if(isset($_POST['csv'])){
                echo "<div>";
                echo "id;;; user;;; email;;; what is it about;;;  how it works;;; when it works;;; time;;; session;;; idea date  <br/>";

                foreach($conn->query($sql) as $row) { 
                  echo "{$row['member_id']};;; {$row['username']};;; {$row['email']};;; {$row['what_about']};;; {$row['how_it_works']};;; {$row['when_it_works']};;; {$row['time']};;; {$row['session']};;; {$row['idea_date']} <br/>";   
              } 
              echo "</div>"; }
              ?>
               ...
               <?php  if(isset($_POST['removed'])){
                echo "<h3>removed ideas</h3>";
                  ...
                  $sql6 =("SELECT member.id , email, username, member_id , removed_idea.id,  what_about, how_it_works, when_it_works  FROM removed_idea , member  where member.id = member_id ORDER BY removed_idea.id DESC ");
                ...
                foreach($conn->query($sql6) as $row) { 
                  echo "<p style='word-wrap: break-word'> 
                            {$row['member_id']}, {$row['username']}, {$row['email']},  {$row['what_about']}, {$row['how_it_works']}, {$row['when_it_works']} 
                        </p>";   
              } 
              echo "</div>"; 
        } ?>
