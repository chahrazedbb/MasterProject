...
elseif  (isset($_POST['register'])){
    ...
    if($row['num'] > 0){
       ...
       $stm = $conn->query("SELECT Vsession , count(id) as ids FROM member GROUP BY Vsession ORDER BY id DESC LIMIT 1") ;
        ...
        if($row["ids"] == 0){
            ...
            $vsession = $row["Vsession"] ;
        }else{
            $vsession = $row["Vsession"] + 1;
        }

        $passwordHash = password_hash($pass, PASSWORD_BCRYPT, array("cost" => 12));
        
        $sql = "INSERT INTO member (username,email, password, Vsession)
                 VALUES (:username, :email, :password, :Vsession)";
        $stmt = $conn->prepare($sql);

        $stmt->bindValue(':username', $username);
        $stmt->bindValue(':email', $email);
        $stmt->bindValue(':password', $passwordHash);
        $stmt->bindValue(':Vsession', $vsession);
